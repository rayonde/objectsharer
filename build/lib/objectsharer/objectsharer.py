...
class ObjectSharer(object):
    ...
    def call(self, client, objuid, func_name, *args, **kwargs):
        ...
        async_value = kwargs.pop('async', False) or (callback is not None) or is_signal
        ...
        if DEBUG:
            logger.debug('Sending call %d to %s: %s.%s(%s,%s), async=%s', callid, client, objuid, func_name, misc.ellipsize(str(args)), misc.ellipsize(str(kwargs)), async_value)
...
#            print 'Calling receive sig, uid=%s, signame %s, args %s, kwargs %s' % (uid, signame, args, kwargs)
    ...
    def request_client_proxy(self, uid, async_value=False):
        '''
